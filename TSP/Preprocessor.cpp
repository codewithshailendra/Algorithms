...
const string data{ R"(C:\Users\n419\Repos\Data)" };
const string home{ R"(C:\Users\n419\Repos\TSP2\TSP2\Client)" };
const string path{ R"(..\..\..\Data\)" };
...
public:
	...
	int writeSet() {
		string name{ path + R"(comb)" + std::to_string(comboLength_) + ".txt" };
		cout << name << endl;
		ofstream fs;
		fs.open(name);
		if (!fs) {
	...
	std::unique_ptr<std::vector<std::set<int>>> readFile(int m) {
		auto result = std::make_unique<std::vector<std::set<int >>>();
		string name{ path + R"(comb)" + std::to_string(m) + ".txt" };
		std::cout << name << endl;
		ifstream ifs;
		ifs.open(name);
		if (!ifs.is_open()) {
			cout << "Unable to open " << name << " for reading " << endl;
			return result;
		}
		string line;
...
void writeFiles() {
	for (int i{ 1 }; i <= 25; ++i) {
		Combinations C(25, i);
		int combSize = C.writeSet();
		cout << "Found " << combSize << " combinations of length " << i << endl;
	}
}
